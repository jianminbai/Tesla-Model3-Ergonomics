# 插图索引

本页面列出当前项目已有 SVG 插图及推荐引用位置。

---

## 01 压力分布示意图

文件：

```text
assets/diagrams/svg/01_pressure_distribution.svg
```

预览：

![压力分布](../assets/diagrams/svg/01_pressure_distribution.svg)

推荐章节：

- 第一章 驾驶为什么会让人疼
- 第九章 实验系统与量化记录

引用：

```markdown
![压力不是消失，而是重新分配](../assets/diagrams/svg/01_pressure_distribution.svg)
```

---

## 02 骨盆三姿态受力图

文件：

```text
assets/diagrams/svg/02_pelvis_postures.svg
```

预览：

![骨盆三姿态](../assets/diagrams/svg/02_pelvis_postures.svg)

推荐章节：

- 第二章 骨盆决定受力

引用：

```markdown
![骨盆后倾、中立、过度前倾](../assets/diagrams/svg/02_pelvis_postures.svg)
```

---

## 03 Model 3 坐垫俯视结构图

文件：

```text
assets/diagrams/svg/03_model3_seat_top_view.svg
```

预览：

![Model 3 坐垫俯视结构](../assets/diagrams/svg/03_model3_seat_top_view.svg)

推荐章节：

- 第四章 Tesla Model 3 座椅结构分析
- 第七章 症状决策树

引用：

```markdown
![Model 3 坐垫俯视结构](../assets/diagrams/svg/03_model3_seat_top_view.svg)
```

---

## 04 座椅高度变化受力图

文件：

```text
assets/diagrams/svg/04_seat_height_force_change.svg
```

预览：

![座椅高度变化](../assets/diagrams/svg/04_seat_height_force_change.svg)

推荐章节：

- 第四章 Tesla Model 3 座椅结构分析
- 第五章 座椅调节流程

引用：

```markdown
![座椅高度变化对髋膝关系与受力的影响](../assets/diagrams/svg/04_seat_height_force_change.svg)
```

---

## 05 右腿踩油门动态负荷链

文件：

```text
assets/diagrams/svg/05_pedal_dynamic_load_chain.svg
```

预览：

![右腿踩油门动态负荷链](../assets/diagrams/svg/05_pedal_dynamic_load_chain.svg)

推荐章节：

- 第六章 踏板几何与右腿疲劳

引用：

```markdown
![右腿踩油门动态负荷链](../assets/diagrams/svg/05_pedal_dynamic_load_chain.svg)
```

---

## 06 症状快速决策总览

文件：

```text
assets/diagrams/svg/06_symptom_decision_overview.svg
```

预览：

![症状快速决策总览](../assets/diagrams/svg/06_symptom_decision_overview.svg)

推荐章节：

- 第七章 症状决策树
- docs/quick_diagnosis_table.md

引用：

```markdown
![症状快速决策总览](../assets/diagrams/svg/06_symptom_decision_overview.svg)
```

---

## 07 座椅调整工程实验流程

文件：

```text
assets/diagrams/svg/07_experiment_system_flow.svg
```

预览：

![实验系统流程](../assets/diagrams/svg/07_experiment_system_flow.svg)

推荐章节：

- 第九章 实验系统与量化记录
- docs/experiment_system.md

引用：

```markdown
![座椅调整工程实验流程](../assets/diagrams/svg/07_experiment_system_flow.svg)
```

---

## 08 车座、办公椅、身体状态三角模型

文件：

```text
assets/diagrams/svg/08_rehab_office_triangle.svg
```

预览：

![车座办公椅身体状态三角模型](../assets/diagrams/svg/08_rehab_office_triangle.svg)

推荐章节：

- 第八章 康复与办公椅联动

引用：

```markdown
![车座、办公椅、身体组织状态三角模型](../assets/diagrams/svg/08_rehab_office_triangle.svg)
```

---

# 真实感图片索引

真实感图片用于帮助读者把抽象受力关系放回车内、办公和身体活动场景。它们不替代 SVG 受力图，而是作为章节开头或关键小节的视觉引导。

## R01 真实感座椅压力分布

文件：

```text
assets/images/realistic/01_realistic_pressure_distribution.png
```

预览：

![真实感座椅压力分布](../assets/images/realistic/01_realistic_pressure_distribution.png)

推荐章节：

- 第一章 驾驶为什么会让人疼
- 第三章 坐骨与软组织受力

---

## R02 真实感骨盆姿态对比

文件：

```text
assets/images/realistic/02_realistic_pelvis_postures.png
```

预览：

![真实感骨盆姿态对比](../assets/images/realistic/02_realistic_pelvis_postures.png)

推荐章节：

- 第二章 骨盆决定受力
- 第五章 座椅调节流程

---

## R03 真实感右腿踏板负荷链

文件：

```text
assets/images/realistic/03_realistic_pedal_load_chain.png
```

预览：

![真实感右腿踏板负荷链](../assets/images/realistic/03_realistic_pedal_load_chain.png)

推荐章节：

- 第六章 踏板几何与右腿疲劳

---

## R04 真实感车座办公椅身体状态联动

文件：

```text
assets/images/realistic/04_realistic_seat_office_body_triangle.png
```

预览：

![真实感车座办公椅身体状态联动](../assets/images/realistic/04_realistic_seat_office_body_triangle.png)

推荐章节：

- 第八章 康复与办公椅联动
- docs/office_chair_checklist.md
